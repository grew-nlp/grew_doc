...
We call here "relation table" a table like the ones which are available through Grew-match: [example on UD_French-PUD, version 2.12](http://universal.grew.fr/meta/UD_French-PUD@2.12_table.html) (select a relation on the left).
...
For this example, we suppose that we have a subfolder `data` which contains the file `fr_pud-ud-test.conllu` (the version 2.12 of the corpus **UD_French-PUD** which can be downloaded [here](https://raw.githubusercontent.com/UniversalDependencies/UD_French-PUD/r2.12/fr_pud-ud-test.conllu)).
