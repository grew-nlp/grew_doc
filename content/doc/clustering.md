...
#### Examples
  * List lemmas of auxiliaries in **UD_Polish-LFG** {{< tryit "https://universal.grew.fr/?corpus=UD_Polish-LFG@2.17&request=pattern { X [upos=AUX] }&clust1_key=X.lemma">}}
  * List `VerbForm` of `VERB` without `nsubj` in **UD_German-GSD** {{< tryit "https://universal.grew.fr/?corpus=UD_German-GSD@2.17&request=pattern { X [upos=VERB] }%0Dwithout { X -[1=nsubj]-> Y }&clust1_key=X.VerbForm">}}
  * Find the huge number of `form` associated to the lemma _saada_ in **UD_Finnish-FTB**{{< tryit "https://universal.grew.fr/?corpus=UD_Finnish-FTB@2.17&request=pattern { X [lemma=\"saada\"] }&clust1_key=X.form">}}
...
#### Example
  * List sub-relations used with `acl` relation in **UD_Swedish-Talbanken** {{< tryit "https://universal.grew.fr/?corpus=UD_Swedish-Talbanken@2.17&request=pattern { e: X -[1=acl]-> Y }&clust1_key=e.2">}}
...
#### Example
  * List relations used for auxiliaries in **UD_Italian-ParTUT** {{< tryit "https://universal.grew.fr/?corpus=UD_Italian-ParTUT@2.17&request=pattern { e:X -> Y; Y [upos=AUX] }&clust1_key=e.label">}}
...
#### Examples
  * Observe the length of the `amod` relation in **UD_Korean-PUD**{{< tryit "https://universal.grew.fr/?corpus=UD_Korean-PUD@2.17&request=pattern { e: X -[amod]-> Y }&clust1_key=e.length">}}
  * Observe the relative positions of `nsubj` related tokens in **UD_Naija-NSC** {{< tryit "https://universal.grew.fr/?corpus=UD_Naija-NSC@2.17&request=pattern { e: X -[nsubj]-> Y }&clust1_key=e.delta">}}
...
🆕 in Version 1.16. Similarly to the [new syntax for request](../request#constraints-on-distance-between-two-nodes), it is possible to cluster on the distance between two nodes:
 - `length(X,Y)` absolute distance between `X` and `Y` {{< tryit "https://universal.grew.fr/?corpus=UD_Galician-PUD@2.17&request=pattern { X -[amod]-> Y }&clustering=length(X,Y)" >}}
 - `delta(X,Y)` relative distance between `X` and `Y` {{< tryit "https://universal.grew.fr/?corpus=UD_Galician-PUD@2.17&request=pattern { X -[amod]-> Y }&clustering=delta(X,Y)" >}}
...
For example, you can cluster on the relative distance between a subject and an object that depend on the same governor: {{< tryit "https://universal.grew.fr/?corpus=UD_Galician-PUD@2.17&request=pattern { X -[nsubj]-> S; X -[obj]-> O }&clustering=delta(S,O)" >}}
...
#### Example with `ExtPos`
On **UD_French-GSD**, when searching for POS of a dependent of the `case` relation with the request `pattern { X -[case]-> Y }`, the clustering key `Y.upos` reports 8 clusters {{< tryit "https://universal.grew.fr/?corpus=UD_French-GSD@2.17&request=pattern { X -[case]-> Y }&clustering=Y.upos" >}} (use the `Count` button to see all clusters) and the clustering key `Y.ExtPos/upos` reports the more regular set of 3 clusters {{< tryit "https://universal.grew.fr/?corpus=UD_French-GSD@2.17&request=pattern { X -[case]-> Y }&clustering=Y.ExtPos/upos" >}}.
...
{{< tryit "https://universal.grew.fr/?corpus=UD_French-GSD@2.17&request=pattern { X -[amod]-> Y ; X.Gender <> Y.Gender}&clust1_key=X.CorrectGender/Gender&clust2_key=Y.CorrectGender/Gender" >}}, we can observe in more detail the `Gender` agreement between two nodes related by `amod`.
...
{{< tryit "https://universal.grew.fr/?corpus=UD_Latin-Perseus@2.17&request=pattern { %0A  V[upos=VERB];%0A  V -[nsubj]-> S;%0A  V -[obj]-> O;%0A }&clustering=V%23S%23O" >}} on `UD_Latin-Perseus`.
...
{{< tryit "https://universal.grew.fr/?corpus=UD_French-GSD@2.17&request=pattern { %0A  HEAD [];%0A  COP [upos=AUX]; HEAD -[cop]-> COP;%0A  ADP [upos=ADP]; HEAD -[case|mark]-> ADP;%0A}&clustering=HEAD%23COP%23ADP" >}} on `UD_French-GSD` with the clustering key `HEAD#COP#ADP`.
...
we can observe how the bigram is annotated: {{< tryit "https://universal.grew.fr/?corpus=UD_German-GSD@2.17&request=pattern { X [upos=DET]; Y [upos=NOUN]; X < Y }&clustering=Y -> X" >}} on `UD_German-GSD`.
...
we can observe how the bigram NOUN-NOUN is annotated: {{< tryit "https://universal.grew.fr/?corpus=UD_Chinese-GSD@2.17&request=pattern { X [upos=NOUN]; Y [upos=NOUN]; X < Y }&clustering=X <-> Y" >}} on `UD_Chinese-GSD` or {{< tryit "https://universal.grew.fr/?corpus=bUD_English-GUM@2.17&request=pattern { X [upos=NOUN]; Y [upos=NOUN]; X < Y }&clustering=X <-> Y" >}} on `bUD_English-GUM` (`bUD` is the version of the treebank whitout the enhanced dependency layer).
...
Example:
 - `speaker_sex` and `Person` use in Naija: {{< tryit "https://universal.grew.fr/?corpus=SUD_Naija-NSC@2.17&request=pattern { X [Person] }&clust1_key=X.Person&clust2_key=meta.speaker_sex" >}}
...
### Examples
  ...
  - Is `advcl` left-headed in **UD_Hungarian-Szeged**? {{< tryit "https://universal.grew.fr/?corpus=UD_Hungarian-Szeged@2.17&request=pattern { X -[advcl]-> Y }&whether=X << Y" >}}
  - In **UD_English-GUM**, how often does the relation `expl` appear with or without an `nsubj` relation with the same head? {{< tryit "https://universal.grew.fr/?corpus=UD_English-GUM@2.17&request=pattern { X -[1=expl]-> Y }&whether=X -[1=nsubj]-> S" >}}
  - How often the text starts with "And": {{< tryit "https://universal.grew.fr/?corpus=UD_English-EWT@2.17&request= &clust1_whether=meta.text = re\"And.*\"" >}}.
  - In **UD_French-GSD**, there are 618 left-headed `nsubj` (or subtypes):
    - How often is it in an interrogative sentences? {{< tryit "https://universal.grew.fr/?corpus=UD_French-GSD@2.17&request=pattern { X -[1=nsubj]-> Y; X << Y }&whether=P [lemma=%22?%22]" >}} (NB: We approximate interrogative with the presence of "?")
    - How often is it in an relative clause? {{< tryit "https://universal.grew.fr/?corpus=UD_French-GSD@2.17&request=pattern { X -[1=nsubj]-> Y; X << Y }&whether=H -[acl:relcl]-> X" >}}
    - How often is there an expletive subject? {{< tryit "https://universal.grew.fr/?corpus=UD_French-GSD@2.17&request=pattern { X -[1=nsubj]-> Y; X << Y }&whether=X -[expl:subj]-> E" >}}
